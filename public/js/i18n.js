...
window.I18N = (function () {
  const DICT = {
    ...
    "confirm.folderMessage": { en: "You are about to upload {count} files (~{size} MB). This may take a moment depending on your connection.", fa: "قراره {count} فایل (حدود {size} مگابایت) آپلود شود. بسته به سرعت اینت��نتت��ن ممک�� است کمی طول بکشد." },
    ...
    "g.adv.t": { en: "Suggestions tab", fa: "تب پیشنهادها" },
    "g.adv.d": { en: "A rule-based, offline advisor that reads your report and writes prioritized, bilingual suggestions (with a typewriter effect) for fixing complexity, duplication, cycles, HTML/CSS and SQL problems. It always speaks the site's current language automatically.", fa: "مشاور آفلاین و مبتنی بر قانون که گزارش شما را می‌خواند و پیشنهادهای اولویت‌بندی‌شده و دوزبانه (با افکت تایپ‌رایتری) برای رفع پیچیدگی، تکرار، حلقه‌ها و مشکلات HTML/CSS/SQL می‌نویسد. همیشه به زبان فعلی سایت و به‌صورت خودکار صحبت می‌کند." },
    "tab.advisor": { en: "Suggestions", fa: "پیشنهادها" },
    "adv.title": { en: "Smart suggestions", fa: "پیشنهادهای هوشمند" },
    "adv.subtitle": { en: "Rule-based tips to improve this project - no AI involved.", fa: "نکاتی مبتنی بر قوانین برای بهتر شدن این پروژه - بدون هوش مصنوعی." },
    "adv.empty": { en: "No notable issues found. Nice work!", fa: "هیچ مشکل قابل‌توجهی پیدا نشد. کارت خوب بوده!" },
    "adv.replay": { en: "Replay", fa: "پخش دوباره" },
    "adv.sevHigh": { en: "High impact", fa: "تأثیر بالا" },
    "adv.sevMed": { en: "Medium", fa: "متوسط" },
    "adv.sevLow": { en: "Polish", fa: "جزئی" },
    "adv.sevOk": { en: "Great job", fa: "عالی بود" },
    "adv.cxTitle": { en: "Break up complex functions ({n} found)", fa: "شکستن توابع پیچیده ({n} مورد)" },
    "adv.cxBody": { en: "\"{name}\" in {file} has cyclomatic complexity {cc} (recommended max {limit}). Split it into smaller single-purpose functions, replace nested if/else chains with early returns, and move each independent rule into its own well-named helper. Start with the biggest branch inside it.", fa: "«{name}» در {file} پیچیدگی سیکلوماتیک {cc} دارد (حداکثر توصیه‌شده {limit}). آن را به توابع کوچک‌تر و تک‌مسئولیتی بشکنید، زنجیره‌های if/else تودرتو را با بازگشت زودهنگام (early return) جایگزین کنید و هر قانون مستقل را در یک تابع کمکی جدا با نام مشخص قرار دهید. از بزرگ‌ترین شاخه‌ی داخل آن شروع کنید." },
    "adv.longTitle": { en: "Shorten long functions ({n} found)", fa: "کوتاه‌کردن توابع طولانی ({n} مورد)" },
    "adv.longBody": { en: "\"{name}\" in {file} is {lines} lines long (recommended max {limit}). Extract each logical step (validation, computation, output) into its own helper - short functions are easier to test, reuse and review.", fa: "«{name}» در {file} حدود {lines} خط طول دارد (حداکثر توصیه‌شده {limit}). هر گام منطقی (اعتبارسنجی، محاسبه، خروجی) را در یک تابع کمکی جدا قرار دهید - توابع کوتاه تست، بازاستفاده و بازبینی راحت‌تری دارند." },
    "adv.nestTitle": { en: "Flatten deeply nested code ({n} found)", fa: "ساده‌کردن کد تودرتوی عمیق ({n} مورد)" },
    "adv.nestBody": { en: "\"{name}\" in {file} nests {depth} levels deep. Use guard clauses (return early on invalid input), extract inner loops into helpers, and invert conditions to keep the happy path at the top level.", fa: "«{name}» در {file} تا عمق {depth} سطح تودرتو شده است. از guard clause (بازگشت زودهنگام برای ورودی نامعتبر) استفاده کنید، حلقه‌های داخلی را در توابع کمکی جدا کنید و شرط‌ها را معکوس کنید تا مسیر اصلی در سطح بالا بماند." },
    "adv.fileTitle": { en: "Split oversized files ({n} found)", fa: "شکستن فایل‌های بزرگ ({n} مورد)" },
    "adv.fileBody": { en: "{file} is {lines} lines (recommended max {limit}). Group related functions and move each group into its own module - one responsibility per file makes navigation and code review much faster.", fa: "{file} حدود {lines} خط دارد (حداکثر توصیه‌شده {limit}). توابع مرتبط را گروه‌بندی کنید و هر گروه را به ماژول جدا منتقل کنید - یک مسئولیت در هر فایل، پیمایش و بازبینی کد را بسیار سریع‌تر می‌کند." },
    "adv.dupTitle": { en: "Remove duplicated code ({pct}% of lines)", fa: "حذف کد تکراری ({pct}% خطوط)" },
    "adv.dupBody": { en: "{count} repeated block(s) were found. Extract the shared logic into one function or module and import it everywhere it is needed - every copy is a place where a future fix can be forgotten. The Duplicates tab lists the exact locations.", fa: "{count} بلوک تکراری پیدا شد. منطق مشترک را در یک تابع یا ماژول جدا کنید و هرجا لازم است ایمپورت کنید - هر نسخه‌ی تکراری جایی است که ممکن است یک اصلاح آینده در آن فراموش شود. تب Duplicates مکان دقیق آن‌ها را نشان می‌دهد." },
    "adv.cycleTitle": { en: "Break circular dependencies ({n} cycle(s))", fa: "باز کردن وابستگی‌های حلقوی ({n} حلقه)" },
    "adv.cycleBody": { en: "Files import each other in a loop: {chain}. Extract what both sides need into a third shared module, or use dependency injection so one side no longer imports the other. Cycles make code impossible to reuse in isolation.", fa: "فایل‌ها یکدیگر را به‌صورت حلقه‌ای import می‌کنند: {chain}. آنچه هر دو طرف نیاز دارند را در یک ماژول مشترک سوم جدا کنید یا از dependency injection استفاده کنید تا یک طرف دیگر نیازی به import طرف دیگر نداشته باشد. حلقه‌ها استفاده‌ی مجزا از کد را غیرممکن می‌کنند." },
    "adv.htmlAlt": { en: "{n} image(s) without alt text", fa: "{n} تصویر بدون متن alt" },
    "adv.htmlDep": { en: "{n} deprecated tag(s)", fa: "{n} تگ منسوخ" },
    "adv.htmlInl": { en: "{n} inline event handler(s)", fa: "{n} هندلر رویداد درون‌خطی" },
    "adv.htmlDupId": { en: "{n} duplicate id(s)", fa: "{n} شناسه‌ی تکراری" },
    "adv.htmlTitle": { en: "Clean up the HTML", fa: "پاک‌سازی HTML" },
    "adv.htmlBody": { en: "Found: {parts}. Add alt attributes for accessibility and SEO, replace deprecated tags with CSS, and move onclick handlers into addEventListener calls in your scripts. Details are in the Overview issue tables.", fa: "موارد پیدا شده: {parts}. برای دسترس‌پذیری و سئو attribute alt اضافه کنید، تگ‌های منسوخ را با CSS جایگزین کنید و هندلرهای onclick را به addEventListener در اسکریپت‌ها منتقل کنید. جزئیات در جدول‌های Overview موجود است." },
    "adv.cssImp": { en: "{n} !important", fa: "{n} مورد !important" },
    "adv.cssDupProp": { en: "{n} duplicate propertie(s)", fa: "{n} ویژگی تکراری" },
    "adv.cssDupSel": { en: "{n} duplicate selector(s)", fa: "{n} سلکتور تکراری" },
    "adv.cssZ": { en: "{n} extreme z-index value(s)", fa: "{n} مقدار z-index بسیار بالا" },
    "adv.cssTitle": { en: "Tidy up the CSS", fa: "مرتب‌سازی CSS" },
    "adv.cssBody": { en: "Found: {parts}. Prefer more specific selectors over !important, merge duplicate rules into one place, and keep z-index values in a small ordered scale (1, 10, 100) so stacking stays predictable.", fa: "موارد پیدا شده: {parts}. به‌جای !important از سلکتورهای دقیق‌تر استفاده کنید، قوانین تکراری را یکجا ادغام کنید و مقادیر z-index را در یک مقیاس مرتب کوچک (۱، ۱۰، ۱۰۰) نگه دارید تا ترتیب لایه‌ها قابل‌پیش‌بینی بماند." },
    "adv.sqlDangerTitle": { en: "Guard dangerous SQL statements ({n} found)", fa: "محافظت از دستورات خطرناک SQL ({n} مورد)" },
    "adv.sqlDangerBody": { en: "A DELETE or UPDATE without WHERE was found in {file} (line {line}) - it touches every row in the table. Add an explicit WHERE clause, and consider wrapping bulk changes in a transaction so they can be rolled back.", fa: "یک DELETE یا UPDATE بدون WHERE در {file} (خط {line}) پیدا شد - این دستور روی همه‌ی ردیف‌های جدول اثر می‌گذارد. یک WHERE صریح اضافه کنید و تغییرات دسته‌ای را در یک تراکنش (transaction) قرار دهید تا قابل بازگشت باشد." },
    "adv.sqlStarTitle": { en: "Replace SELECT * ({n} found)", fa: "جایگزینی SELECT * ({n} مورد)" },
    "adv.sqlStarBody": { en: "List only the columns you actually need. SELECT * transfers unnecessary data, breaks silently when the schema changes, and prevents covering indexes from being used.", fa: "فقط ستون‌هایی را که واقعاً لازم دارید انتخاب کنید. SELECT * داده‌ی غیرضروری منتقل می‌کند، با تغییر schema به‌آرامی خراب می‌شود و مانع استفاده از covering index می‌شود." },
    "adv.parseTitle": { en: "{n} file(s) could not be fully parsed", fa: "{n} فایل به‌طور کامل تحلیل نشد" },
    "adv.parseBody": { en: "Some files were skipped or partially analyzed due to syntax the parser could not read. Check them for syntax errors - fixing them also makes the rest of this report more accurate.", fa: "برخی فایل‌ها به‌دلیل نحوی که تحلیل‌گر نتوانست بخواند، رد شدند یا ناقص تحلیل شدند. آن‌ها را از نظر خطای نحوی بررسی کنید - رفع آن‌ها باقی گزارش را هم دقیق‌تر می‌کند." },
    "adv.cleanTitle": { en: "Your code is in great shape!", fa: "کد شما وضعیت خوبی دارد!" },
    "adv.cleanBody": { en: "No significant problems were found: functions are small, duplication is low and there are no dependency cycles. Keep it that way - re-run the analysis after big changes, keep functions under the limits, and add tests for the most complex parts.", fa: "مشکل مهمی پیدا نشد: توابع کوچک هستند، تکرار کد کم است و هیچ حلقه وابستگی وجود ندارد. همین روند را حفظ کنید - بعد از تغییرات بزرگ دوباره تحلیل کنید، توابع را زیر حد آستانه نگه دارید و برای پیچیده‌ترین بخش‌ها تست بنویسید." },
    "kg.node.thing": { en: "node", fa: "گره" },
    "kg.node.summary": { en: "\u201c{name}\u201d is a {type} with {total} connection(s) in this view: {inc} incoming, {out} outgoing.", fa: "«{name}» یک {type} با {total} ارتباط در این نما است: {inc} ورودی، {out} خروجی." },
    "kg.node.file": { en: "Defined in:", fa: "تعریف‌شده در:" },
    "kg.node.files": { en: "Connected files:", fa: "فایل‌های مرتبط:" },
    "kg.node.in": { en: "Incoming", fa: "ورودی" },
    "kg.node.out": { en: "Outgoing", fa: "خروجی" },
    "kg.node.none": { en: "No connections in the current view.", fa: "در نمای فعلی ارتباطی وجود ندارد." },
    "sql.chipFromCode": { en: "Inferred from code", fa: "استخراج‌شده از کد" },
    "sql.chipQueries": { en: "Queries", fa: "کوئری‌ها" },
    "sql.inferredBadge": { en: "inferred", fa: "استنتاج‌شده" },
    "sql.noCols": { en: "No columns detected", fa: "ستونی شناسایی نشد" },
